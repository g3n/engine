...
func (g *GLTF) LoadNode(nodeIdx int) (core.INode, error) {
	...
	if nodeData.Mesh != nil {
		...
		if nodeData.Skin != nil {
			...
			if len(children) == 1 {
				rm := graphic.NewRiggedMesh(children[0].(*graphic.Mesh))
				rm.SetSkeleton(skeleton)
				in = rm
			} else {
				// Multiple primitives: give each its own RiggedMesh sharing the same skeleton
				container := core.NewNode()
				for _, child := range children {
					rm := graphic.NewRiggedMesh(child.(*graphic.Mesh))
					rm.SetSkeleton(skeleton)
					container.Add(rm)
				}
				in = container
...
func (g *GLTF) LoadCamera(camIdx int) (core.INode, error) {
	...
	if camData.Type == "orthographic" {
		...
		cam := camera.NewOrthographic(desc.Xmag/desc.Ymag, desc.Znear, desc.Zfar, desc.Ymag, camera.Vertical)
		return cam, nil
	...
	if matData.Extensions != nil {
		for ext, extData := range matData.Extensions {
			if ext == KhrMaterialsCommon {
				...
				// Fall through to PBR below; the unlit flag is advisory only.
			} else if ext == VRM {
				// VRM root-level material extension; ignore and use PBR data.
			} else {
				log.Warn("unknown material extension %s, falling back to PBR", ext)
			}
		}
	}
	// Load PBR if no other material type was resolved
	if !hasCommon && imat == nil {
